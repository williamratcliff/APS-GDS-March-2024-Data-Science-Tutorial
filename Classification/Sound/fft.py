...
def write_fft(fft_features, fn):
    ...
    #print("data")
    #print(data_fn)
    #np.save("bob", fft_features)
    #np.save("bob", np.array([1,23]))
    np.save(Path(data_fn), fft_features)
...
if __name__ == "__main__":
    #print(os.path.join(sys.argv[1], "*.wav"))
    #GENRE_LIST.remove("classical")
    for genre in GENRE_LIST:
        #test_dir=r"C:\Users\ylem\Box\APS March Meeting Tutorial 2024\Sound\archive\Data\genres_original\blues"
        test_dir=Path(GENRE_DIR)/genre
        myfiles=glob.glob(os.path.join(test_dir, "*.wav"))
        #print('\n'.join(myfiles))    
        #for fn in glob.glob(os.path.join(sys.argv[1], "*.wav"))
        for fn in glob.glob(os.path.join(test_dir, "*.wav")):
            #print(fn)
            create_fft(fn)
